...
- **重要性**: 🔴 高
- **难度**: 🟡 中
- **说明**: 当前每个订阅者独立消费同一份消息。消费者组允许多个消费者分摊一个 Topic 的消息，每条消息只被组内一个消费者处理（类似 Kafka 的 Consumer Group）。
- **涉及模块**: `mq/topic_manager`, `mq/broker`, `storage/`

### 3. 消息 ACK + 重试机制
- **重要性**: 🔴 高
- **难度**: 🟢 低
- **说明**: 当前 ACK 仅记录 offset，没有重试。应支持：
  - 未 ACK 的消息进入重试队列
  - 最大重试次数限制
  - 死信队列（DLQ）存放最终失败的消息
...
- **难度**: 🟢 低
- **说明**: 
  - **生产端**: 客户端累积多条消息一次性发送，减少系统调用
  - **消费端**: Broker 批量推送消息给订阅者，降低网络往返
- **涉及模块**: `network/tcp_client`, `mq/broker`, `protocol/`

### 8. 背压与限流（Backpressure / Flow Control）
- **重要性**: 🟡 中
- **难度**: 🟡 中
- **说明**: 当消费者处理速度慢于生产者时，防止内存无限增长。实现方式：
  - 基于 Ring Buffer 剩余容量的发布限流
  - 消费者端令牌桶/漏桶限流
  - TCP 窗口级别的流量控制
...
- **重要性**: 🟡 中
- **难度**: 🟢 低
- **说明**: 对 payload 进行 LZ4 / Snappy / Zstd 压缩，减少网络带宽和存储占用。帧头中增加压缩类型字段。
- **涉及模块**: `protocol/frame_codec`, `storage/`

### 10. 消息过期与 TTL
- **重要性**: 🟡 中
- **难度**: 🟢 低
- **说明**: 为消息设置存活时间，过期后自动清理。适用于日志类 Topic 不需要永久保留的场景。
...
- **重要性**: 🟡 中
- **难度**: 🟢 低
- **说明**: 当前 Ring Buffer 大小、最大消息数等均为硬编码。支持通过配置文件或管理接口动态调整 Topic 参数（缓冲区大小、TTL、压缩开关等）。
- **涉及模块**: `storage/`, `mq/topic_manager`, `common/`
